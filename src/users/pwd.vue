      ...
      <mu-container>
        <mu-form :model="form" ref="pform" class="mu_form">
          ...
          <mu-form-item class="margin_btm_init">
            <mu-button class="btn_bg" full-width @click="register">注册</mu-button>
          </mu-form-item>
          <mu-form-item class="margin_btm_init">
            <span>
              注册即视为同意
              <span class="color_blue" @click="agree">《用户注册协议》</span>
            </span>
          </mu-form-item>
        </mu-form>
      </mu-container>
...
export default {
  ...
  methods: {
    send_code() {
      const _this = this;
      const p = { phoneNumber: _this.mobile };
      let t = 60;
      if (/^1[3|5|7|8|9]\d{9}$/.test(_this.mobile)) {
        if (_this.send) {
          _this.send = false;
